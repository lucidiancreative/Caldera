...
- Wired the React editing actions (Phase 2, part 4): edit (a block-editor modal), delete (with recurring today/all scope), reschedule one-off blocks to another day, and add/delete sub-tasks — all flowing through a new schedule bridge that reuses the vanilla domain logic and persists + notifies; verified that edits round-trip into the React DOM
